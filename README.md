...
**Domain:** `Data / Database Docs` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Schema Documenter** is a standalone, browser-based AI workflow app for **Data / Database Docs**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `schema`<br>Schema | select | Yes | — | Affects input: Schema. |
...
| `db_dialect`<br>DB dialect | text | No | — | Affects customization: DB dialect. |
| `audience`<br>audience | select | No | general | Affects customization: audience. |
| `masking_rules`<br>masking rules | textarea | No | — | Affects customization: masking rules. |
| `naming_convention`<br>naming convention | text | No | — | Affects customization: naming convention. |
| `export_format`<br>export format | select | No | — | Affects customization: export format. |
| `mermaid_html_mode`<br>Mermaid/HTML mode | select | No | — | Affects customization: Mermaid/HTML mode. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9150
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9150`. |
...
```bash
curl http://127.0.0.1:9150/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9150/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9150 uvicorn app.main:app --host 0.0.0.0 --port 9150
```
...
- `plugins/schema-documenter.json` for plugin metadata.
